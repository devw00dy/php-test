...
var scrollHeightStandard = 70;
var MediaCnt = 0;
function fixedHeader() { // 231101 김다우 수정
    let scrollY = window.pageYOffset || document.documentElement.scrollTop;
    const Logo = document.getElementById("LogoBox");
    Logo.style.transition = "0.1s ease";
    const GNB_contents = document.querySelectorAll(".top_category > ul > li");
    const topareaState = document.getElementById("toparea_state");

    const header = document.getElementById("header");
    const TopNavBox = document.getElementById("top_nav_box");

    const TopSubSetion = document.getElementsByClassName("Top_Sub_Section");
    const Mobile_Top_Search = document.getElementById("Mobile_Top_Search"); 
    
	if(scrollY > scrollHeightStandard ) {
        
        for (var i = 0; i < GNB_contents.length; i++) {
          const item = GNB_contents.item(i);
        ...
        if (window.matchMedia("(min-width: 1025px)").matches) { TopNavBox.style.marginTop = "40px";  TopNavBox.style.top = "40px"; scrollHeightStandard = 70; }
        if (window.matchMedia("(max-width: 1025px)").matches) { TopNavBox.style.top = "0px"; TopNavBox.style.marginTop = "0px";scrollHeightStandard = 50; }
        
        
        TopNavBox.style.height = "50px";
        for(var j = 0; j < TopSubSetion.length; j++ ) {
            TopSubSetion[j].style.top = TopNavBox.getBoundingClientRect().bottom + "px";
            TopSubSetion[j].style.marginTop = "80px";
        }
        Mobile_Top_Search.style.margin = "0px 0px 0px";
        Mobile_Top_Search.style.top = "48px";
        
        MediaCnt = 0;
    } 
    else {
        TopNavBox.style.backgroundColor = "#ffffff";
        Mobile_Top_Search.style.margin = "0px";
        Mobile_Top_Search.style.top = "90px";
        ...
        topareaState.style.top = '10px';
        
        TopNavBox.style.marginTop  = '0px';
        TopNavBox.style.height = "70px";
        
        if (window.matchMedia("(max-width: 1025px)").matches && MediaCnt == 1) { TopNavBox.style.top = "40px"; }
        ...
        for(var j = 0; j < TopSubSetion.length; j++ ) {
            TopSubSetion[j].style.top = TopNavBox.getBoundingClientRect().bottom + "px";
            TopSubSetion[j].style.marginTop = "0px";
        }
